...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, white_bg=True, is_train=False, device='cuda'):
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        rgb_map = tensorf(rays_chunk, is_train=is_train, white_bg=white_bg, N_samples=N_samples)
        rgbs.append(rgb_map)
    
    return torch.cat(rgbs)
...
def evaluation(test_dataset,tensorf, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, samples in enumerate(test_dataset.all_rays[0::img_eval_interval]):
        ...
        rgb_map = renderer(rays, tensorf, chunk=4096, N_samples=N_samples, white_bg=white_bg, device=device)
